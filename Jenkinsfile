...
pipeline {
    ...
    stages {
        ...
        stage('Test') {
            steps {
                sh """
                nosetests test --exe --with-doctest --with-xunit --xunit-file test-results.xml --with-coverage --cover-erase --cover-package=pycommon_database/. --cover-min-percentage=91 --cover-html
                """
            }
        }
