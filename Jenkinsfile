    ...
    stages {
        stage('Build') {
            steps {
                ...
                cd test
                nosetests . --exe --with-doctest --with-xunit --xunit-file python_unittest_out.xml --with-coverage --cover-erase --cover-package=../pycommon_database/. --cover-min-percentage=91 --cover-html
                coverage xml
                """
            }
        }
