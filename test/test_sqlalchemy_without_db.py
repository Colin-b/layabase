...
def test_health_details_failure(disconnected_database, monkeypatch):
    ...
    monkeypatch.setattr(disconnected_database.metadata.bind.dialect, "do_ping", lambda x: False)
    assert database.health_details(disconnected_database) == (
        "fail",
        {
            "sqlite:select": {
...
def test_health_details_failure_due_to_exception(disconnected_database, monkeypatch):
    ...
    monkeypatch.setattr(disconnected_database.metadata.bind.dialect, "do_ping", raise_exception)
    assert database.health_details(disconnected_database) == (
        "fail",
        {
            "sqlite:select": {
